...
def run(url=None, presetFilters=None, startprice=None):
    ...
    while end is False:
        ...
        for i in range(1, 24):  # times out after 25th page, so loop with new price
            if end is True:
                break
            try:
                time.sleep(12)
                ...
                for card in listings:
                    ...
                    card['utcDateTime'] = datetime.utcnow()
                    if len(localStorage) > 5:
                        if current_price < localStorage[-4]['unformattedPrice'] - 50000:
                            localStorage.pop(-1)
                            print('break in loop')
                            end = True
                            break
        ...
        with open("test.json", "w") as save_json:
            json.dump(localStorage, save_json)
        df = pd.DataFrame.from_dict(localStorage, orient='columns')
        df.to_csv('ny_dataTest2.csv', encoding='utf-8')
